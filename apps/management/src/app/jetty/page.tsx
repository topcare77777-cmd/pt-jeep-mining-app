...
export default function JettyManagementPage() {
    ...
    const [userRole, setUserRole] = useState('Port & Shipping')
        ...
        async function initJetty() {
            try {
                ...
                if (isMounted) {
                    ...
                    const division = (profile?.role || 'Port & Shipping').trim()
                        ...
                        if (allPerms && allPerms.length > 0) {
                            ...
                            const matched = allPerms.find((p) => {
                                ...
                                return (
                                    ...
                                    (cleanDiv.includes('jetty') && target.includes('jetty')) ||
                                    (cleanDiv.includes('pelabuhan') && target.includes('pelabuhan'))
                                )
                            })
                    ...
                    if (!isSuperAdmin && grantedKeys.length > 0 && !grantedKeys.includes('jetty')) {
                        alert('Divisi Anda tidak memiliki hak akses ke modul Pelabuhan Jetty.')
                        router.replace('/')
                        return
                    }
        ...
        <div className="min-h-screen bg-[#060c14] text-slate-100 font-sans p-4 md:p-6 select-none">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-xl md:text-2xl font-black tracking-wide text-white">
                                Jetty Port & Ore Barging Terminal PT. JEEP
                            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div className="bg-[#0a1625] border border-[#16273c] rounded-xl p-5 shadow-lg">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Total Bijih Terkirim (Shipped)</h3>
                        ...
                        <tbody className="divide-y divide-[#16273c] text-slate-300">
                            {filteredBarges.length > 0 ? (
                                filteredBarges.map((b) => (
                                    <tr key={b.id} className="hover:bg-[#0c1a2d]/50 transition">
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-[#0a1625] border border-[#1b2e46] rounded-xl p-6 w-full max-w-md shadow-2xl space-y-4">
                        ...
                        <form onSubmit={handleAddBarge} className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="Loading">Loading (Muat Bijih)</option>
                            ...
                            <div>
                                <label className="text-[11px] text-slate-400 block mb-1">Kualitas Bijih (Seam/Spec)</label>
                                <input
                                    type="text"
                                    value={coalQuality}
                                    onChange={(e) => setCoalQuality(e.target.value)}
                                    placeholder="Contoh: Ni 1.8%"
                                    className="w-full bg-[#060c14] border border-[#1b2e46] text-white text-xs px-3 py-2 rounded-lg focus:outline-none focus:border-cyan-400"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Contoh: PT Smelter Nikel Indonesia"
                                    className="w-full bg-[#060c14] border border-[#1b2e46] text-white text-xs px-3 py-2 rounded-lg focus:outline-none focus:border-cyan-400"
                                />
                            </div>
